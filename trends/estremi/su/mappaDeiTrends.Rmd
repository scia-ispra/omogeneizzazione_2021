---
title: Trends delle serie estremi ETCCDI (serie omogeneizzate)
date: "`r Sys.Date()`"
...
Anomalie estremi rispetto al climatologico 61-90

```{r boxplot}
e_charts(  df_anomalieAnnuali %>%
    gather(key="id",value="anomalia",-yy) %>%
    mutate(anomalia=round(anomalia,1)) %>%
    left_join(.,ana ) %>% group_by(id)) %>%
  e_boxplot(serie=anomalia) %>%
  e_tooltip(trigger="item")
```
...
e_charts(  df_anomalieAnnuali %>%
    gather(key="id",value="anomalia",-yy) %>%
    mutate(anomalia=round(anomalia,1)) %>%
    left_join(.,ana ) %>% group_by(yy)) %>%
  e_boxplot(serie=anomalia) %>%
  e_tooltip(trigger="item")
```


Trend delle serie delle anomalie

```{r boxplot_trend}
e_charts(  dfTrend ) %>%
  e_boxplot(serie=trend) %>%
  e_tooltip(trigger="item")
```
