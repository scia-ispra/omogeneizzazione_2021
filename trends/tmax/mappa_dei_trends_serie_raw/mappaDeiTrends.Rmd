---
title: Trends delle serie di tmax (serie raw)
date: "`r Sys.Date()`"
...
read_delim("raw_intersected_tmax_tmintmax_serie_selezionate_cluster_analysis_tmax_1961_2020_senza_limitrofe.csv",delim=";",col_types = YYMMDD_TYPE)->dati
...
c(2,3,4,5,6,10,11,12,13,14,15,1,16,17)->listaClusters

purrr::map(listaClusters,.f=function(CLUSTER){

  df_anomalieAnnuali %>%
    gather(key="id",value="anomalia",-yy) %>%
    mutate(anomalia=round(anomalia,1)) %>%
    left_join(.,ana ) %>%
    filter(cluster==CLUSTER)->temp 
  ...
  if(CLUSTER==max(listaClusters)){
    grafico %>%
      e_connect(paste0("cluster",listaClusters[1:(length(listaClusters)-1)]))->grafico
  }
...
#e_arrange(compact(listaGrafici))

e_arrange(listaGrafici[[1]],listaGrafici[[2]],listaGrafici[[3]],listaGrafici[[4]],listaGrafici[[5]],listaGrafici[[6]],listaGrafici[[7]],listaGrafici[[8]],listaGrafici[[9]],listaGrafici[[10]],listaGrafici[[11]],row=7,cols = 2)
```