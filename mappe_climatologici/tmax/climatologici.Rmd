---
title: Mappa dei valori climatologici tmax
...
purrr::map(unique(utm_sf_df$mm),.f=function(MM){
  ...
  MIN<-0
  BREAKS<-5
  if(MM %in% c(12,1,2)) {MAX<-18; MIN<- -2; BREAKS<-2}
  if(MM %in% c(3,4,5)) {MAX<-26; MIN<- 5; BREAKS<-2}
  if(MM %in% c(6,7,8)) {MAX<-32; MIN<- 12; BREAKS<-2}
  if(MM %in% c(9,10,11)) {MAX<-28; MIN<- 8; BREAKS<-2}
