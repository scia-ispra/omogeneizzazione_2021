---
title: Confronto valori assoluti frost days, serie homog vs serie raw
date: "`r Sys.Date()`"
author: Guido Fioravanti
output: html_document
---

Effetto dell'omogeneizzazione sui `frost days`.

I grafici che seguono confrontano i valori assoluti dei `frost days` per le serie omogeneizzate e le serie raw (non omogeneizzate).
...
read_delim("risultati_fd.csv",delim=";",col_names = TRUE) %>%
  gather(key="id",value="homog",-yy)->hdati

read_delim("raw_risultati_fd.csv",delim=";",col_names = TRUE) %>%
...
Confronto tra `frost days` per decade. Ogni punto dello scatterplot rappresenta il valore calcolato per una singola stazione per un determinato anno: homog vs raw. I valori omogeneizzati sono abbastanza in linea con quelli delle serie raw.
...
I grafici confrontano i valori annuali dei `frost days` per i singoli punti stazione. I cluster mostrano che i valori omogeneizzati sono simili a quelli delle serie, escludendo il cluster 5. 
...
Qui vengono confrontrati i valori climatologici 61-90 utilizzati per calcolare le anomalie annuali. I confronto sono fatti stazione per stazione. E' evidente che i valori climatologici calcolati per le serie omogeneizzate sono molto simili a quelli delle serie raw.

```{r climatologici,include=TRUE}
read_delim("climatologici_fd_climatol1961_1990.csv",delim=",",col_names = TRUE) %>%
  gather(key="id",value="homog")->hdati

read_delim("raw_climatologici_fd_climatol1961_1990.csv",delim=",",col_names = TRUE) %>%
...
leaflet::colorNumeric(palette=scico::scico(n=15,palette="imola"),domain = seq(0,240,20),alpha=1)->paletta
